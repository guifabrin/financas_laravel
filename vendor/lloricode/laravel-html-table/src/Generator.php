...
class Generator
{
	...
	/**
        ...
        private function _header(array $header)
        {
                ...
                $output .= $this->tags['head_cell'].__('common.actions').$this->tags['head_cell_end'];
        ...
        private function _rows_data(array $data)
        {
                ...
                foreach($data as $row1)
                {
                        ...
                        foreach($row1 as $row)
                        {
                                ...
                                if (is_bool($data)){
                                        $data = '<input disabled type="checkbox" '.($data?"checked=true":"").'>';
                                }
        ...
        private function _rows_data_with_model($model, array $fields, condition $limit)
        {
                if($limit == 0)
                {
        ...
        private function _rows_data_with_model_objects($objects, array $fields)
        {
                $data=[];
                foreach($objects as $r)
                {
                        ...
                        $t[]=
                        "<a href='".$_SERVER['REQUEST_URI']."/".$r->id."/edit'>".__('common.edit')."</a>".
                        "<a href='".$_SERVER['REQUEST_URI']."/".$r->id."/confirm'>".__('common.remove')."</a>";
                        if(!is_null($this->optionLinks))
                        {
                                $t[] = $this->_optionLinks($r);
                        }
	...
	private function _optionLinks($model)
	{
        ...
        protected function execute(array $header, $model_or_array, $limit = NULL,array $fields = NULL)
        {
                ...
                } elseif (count($model_or_array) > 0 && is_subclass_of($model_or_array[0], 'Illuminate\Database\Eloquent\Model')){
                        $output .= $this->_rows_data_with_model_objects($model_or_array, $fields);
                }
