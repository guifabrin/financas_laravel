<div class="modal fade" id="model_account_{{ $id }}" tabindex="-1" role="dialog"
     aria-labelledby="{{ $id }}">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" id="{{ $id }}">{{ __('common.import') }} {{ $account->description }}
                </h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="{{ __('common.close') }}">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <form action="{{ url('/account/' . (!$isAccount ? 'invoice/' : '')) }}/uploadOfx" method="POST"
            ...
            <form action="{{ url('/account/' . (!$isAccount ? 'invoice/' : '')) }}/uploadCsv" method="POST"
                  enctype="multipart/form-data">
                @if ($isAccount)
                    <input type="hidden" value="{{ $id }}" name="accountId"/>
                @else
