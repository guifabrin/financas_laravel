...
@section('title')
    {{ $action }} {{ __('transactions.title') }} {{ __('common.in') }}
...
@section('content')
        ...
        @if ($account->is_credit_card)
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    {{ Form::select('invoice_id', $account->getOptionsInvoices(), old('invoice_id', isset($transaction) ? $transaction->invoice_id : $invoice_id), ['class' => 'form-control']) }}
                </div>
            </div>
    </div>
    ...
    old(
        'categories',
        isset($transaction)
            ? $transaction->categories->map(function ($categoryTransaction) {
