...
<div class="container-fluid">
  ...
  <div class="row">
    ...
    @else
      <div class="col-md-8">
        <form class="form-publish" method="POST" action="{{action('\Barryvdh\TranslationManager\Controller@postPublish', $group)}}" data-remote="true" role="form" data-confirm="Are you sure you want to publish the translations group '{{$group}}? This will overwrite existing language files.">
            <input type="hidden" name="_token" value="{{csrf_token()}}">
        ...
        <tbody>
          @foreach($translations as $key => $translation)
            <tr id="{{$key}}">
              ...
              @if($deleteEnabled)
                <td>
                  <a href="{{action('\Barryvdh\TranslationManager\Controller@postDelete', [$group, $key])}}" class="btn btn-danger delete-key" data-confirm="Are you sure you want to delete the translations for '{{$key}}?">
